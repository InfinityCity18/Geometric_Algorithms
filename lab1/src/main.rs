...
fn main() {
    ...
    { // zbiór 1
        ...
        draw_unprocessed("plots/1_up.svg", points.clone(), -1200.0..1200.0).unwrap();
        let (l, r, colin) = draw_processed("plots/1_p.svg", points, -1200.0..1200.0, 1e-14, det_2x2).unwrap();
    ...
    { // zbiór 2
        ...
        draw_unprocessed("plots/2_up.svg", points.clone(), -1e14..1e14).unwrap();
        let (l, r, colin) = draw_processed("plots/2_p.svg", points, -1e14..1e14, 1e-14, det_2x2).unwrap();
        println!("Points on the left: {}", l);
        println!("Points on the right: {}", r);
        println!("Collinear points: {}", colin);
    }
    ...
    { // zbiór 3
        ...
        draw_unprocessed("plots/3_up.svg", points.clone(), -120.0..120.0).unwrap();
        let (l, r, colin) = draw_processed("plots/3_p.svg", points, -120.0..120.0, 1e-14, det_2x2).unwrap();
        println!("Points on the left: {}", l);
        println!("Points on the right: {}", r);
        println!("Collinear points: {}", colin);
    }
    ...
    { // zbiór 4
        ...
        draw_unprocessed("plots/4_up.svg", points.clone(), -1200.0..1200.0).unwrap();
        let (l, r, colin) = draw_processed("plots/4_p.svg", points, -1200.0..1200.0, 1e-16, det_2x2).unwrap();
        println!("Points on the left: {}", l);
        println!("Points on the right: {}", r);
        println!("Collinear points: {}", colin);
    }
...
mod problemset1 {
    ...
    pub fn draw_unprocessed(filename: &str, points: Vec<(Float, Float)>, label_range: Range<Float>) -> Result<(), Box<dyn std::error::Error>> {
        let root = SVGBackend::new(filename, (512, 512)).into_drawing_area();
...
mod problemset4 {
    ...
    pub fn draw_unprocessed(filename: &str, points: Vec<(Float, Float)>, label_range: Range<Float>) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let mut ctx = ChartBuilder::on(&areas[2])
            .x_label_area_size(80)
            .y_label_area_size(80)
            .build_cartesian_2d(label_range.clone(), -70.0..70.0)?;
        ctx
            .configure_mesh()
            .disable_x_mesh()
            .disable_y_mesh()
